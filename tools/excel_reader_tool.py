...
taxonomy_node, excel_row, sheet_name, scope} records. Empty or missing
sheets are skipped silently.
"""
...
def excel_reader_tool(xlsx_path: str) -> str:
    ...
    try:
        ...
        for sheet_name in xf.sheet_names:
            ...
            # scope: "STANDALONE" / "CONSOLIDATED" / None, derived from the
            # canonical sheet name prefix — used by narrative_agent's
            # deterministic verifier to disambiguate a claim's segment.
            scope = None
            if canonical_name.startswith("standalone_"):
                scope = "STANDALONE"
            elif canonical_name.startswith("consolidated_"):
                scope = "CONSOLIDATED"

            # ── Resolve blank current_year/previous_year cells that are
            #    actually unevaluated Excel formulas (see _resolve_formula_cell),
            #    and record each row's real Excel row number + scope so
            #    downstream verifiers can cite an exact cell reference.
            for idx, row in df.iterrows():
                ...
                df.at[idx, "excel_row"]   = excel_row
                df.at[idx, "sheet_name"]  = sheet_name
