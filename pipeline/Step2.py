...
try:
    from pipeline.taxonomy import map_line_item, TAXONOMY, UNRECOGNISED
    TAXONOMY_AVAILABLE = True
    print("✅  taxonomy.py loaded successfully.")
except ImportError:
    TAXONOMY_AVAILABLE = False
    print("⚠️  taxonomy.py not found — falling back to raw string matching.")
...
Return ONLY a valid JSON array, no extra text, no backticks:
...
1. For "Changes in Equity", always structure the line_item name as: "[Reserve Name] - [Row Concept]".
2. Strip out all Roman numerals, letters, or bullet points from the beginning of line_item names.
...
- statement must be exactly one of: "Profit and Loss", "Balance Sheet", "Cash Flow", "Changes in Equity"
- For other statements, section = the bold sub-heading above that line item
...
def build_excel(df, output_path: str):
    ...
    def write_sheet(wb, sheet_name: str, data):
        ...
        for col_letter, label in zip(["A", "B", "C"],
                                      ["Line Item", "FY25 (₹ Lakhs)", "FY24 (₹ Lakhs)"]):
...
def extract_financials(pdf_path: str, output_xlsx: str = "financials.xlsx"):
    ...
    df["report_type"]   = df["report_type"].fillna("Standalone")
    ...
    if TAXONOMY_AVAILABLE:
        from pipeline.taxonomy import map_dataframe
        df = map_dataframe(df)
        unrecognised = df[df["taxonomy_node"] == "UNRECOGNISED"]["line_item"].unique()
        if len(unrecognised):
            print(f"\n⚠️  {len(unrecognised)} unrecognised line item(s) — add to taxonomy.py:")
