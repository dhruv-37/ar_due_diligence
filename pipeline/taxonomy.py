"""
taxonomy.py  —  Context-Aware Financial Rules Engine: Upgrade 3
================================================================
Registry of a fixed set of Internal Taxonomy Nodes (ITNs) that financial
statement line items are classified into.
...
* Schema-driven extraction: taxonomy_node classification now happens
  at the LLM extraction step itself (Step2.call_gemini, via Gemini's
  response_schema, constrained to get_taxonomy_enums()) rather than via
  Python-side string matching after the fact. This module therefore only
  holds the registry + metadata; there is no line-item mapper here.
* UNMAPPED is a registered fallback node the LLM selects directly when a
  line item genuinely doesn't fit any other node.

Usage
-----
    from taxonomy import TaxonomyNode, TAXONOMY, get_taxonomy_enums

    node = TAXONOMY["REVENUE_GROSS"]
    print(node.statement)     # Profit and Loss
    print(node.is_total)      # False
    print(node.sign)          # POSITIVE

    enums = get_taxonomy_enums()   # ["REVENUE_GROSS", ..., "UNMAPPED"]
"""
...
CFI_PURCHASE_OF_PPE = _register(TaxonomyNode(
    name      = "CFI_PURCHASE_OF_PPE",
    statement = Statement.CASH_FLOW,
...
# ──────────────────────────────────────────────────────────────────────────────
# FALLBACK / UNMAPPED NODE
# ──────────────────────────────────────────────────────────────────────────────
# Registered (unlike the old UNRECOGNISED sentinel) so it is a first-class
# member of TAXONOMY / get_taxonomy_enums() -- the LLM extraction schema can
# select it directly when a line item genuinely doesn't fit any node, instead
# of Python-side fuzzy matching guessing at one after the fact.

UNMAPPED = _register(TaxonomyNode(
    name      = "UNMAPPED",
    statement = Statement.PROFIT_AND_LOSS,  # placeholder -- not a real classification
...
def get_taxonomy_enums() -> list[str]:
    """
    Returns every registered node name as a flat list of strings, e.g.
    ["REVENUE_GROSS", "REVENUE_GST_DEDUCTION", ..., "UNMAPPED"].

    Used to build the taxonomy_node enum constraint in the Gemini
    response_schema for schema-driven extraction (Step2.py) and claim
    extraction (narrative_agent.py), so the LLM can only ever return a
    node name that actually exists in the registry.
    """
